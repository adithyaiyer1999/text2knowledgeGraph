...
def addToGraphFromText_(text, json_text):
    query_prompt = "The user will provide a JSON string, which contains the current summary in a JSON tree format. The user will then provide a field Text To Be Added. You must understand the Text To Be Added, and update the JSON format provided with this new information. Your task is to take in the existing json text and append the new paragraph given into the form of json representation into the existing json. You cannot lose information of the old json. Return ONLY in JSON format. Nothing else."
